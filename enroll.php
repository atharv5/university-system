...
if(strlen($_SESSION['login'])==0 or strlen($_SESSION['pcode'])==0)
    ...
    if(isset($_POST['submit']))
    {
        ...
        // check if studentRegno and course already exist
        $check = mysqli_query($con, "SELECT * FROM courseenrolls WHERE studentRegno='$studentregno' AND course='$course'");
        $count = mysqli_num_rows($check);

        if($count > 0) {
            // alert already registered for the course
            echo '<script>alert("Already registered for the course")</script>';
            echo '<script>window.location.href=enroll.php</script>';
        } else {
            // insert into table if studentRegno and course don't exist
...
{ ?>
 ...
 <div class="form-group">
    <label for="studentregno">Student UIN   </label>
